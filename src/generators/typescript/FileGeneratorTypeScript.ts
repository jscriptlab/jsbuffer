...
export default class FileGeneratorTypeScript
  extends CodeStream
  implements IFileGeneratorTypeScript
{
  ...
  readonly #originalImports = new Map<
    FileGeneratorTypeScript,
    {
      identifiers: Set<string>;
      externalModule: boolean;
      fileGenerator: FileGeneratorTypeScript;
    }
  >();
  readonly #fileGenerators = new Map<string, FileGeneratorTypeScript>();
  readonly #parent;
  ...
  //   string,
  //   IMetadataTypeDefinition | INodeTraitDefinition
  // >();
  ...
  public constructor(
    ...
    }: IFileGeneratorOptions<FileGeneratorTypeScript>
  ) {
    super(undefined, {
      indentationSize
    });
  ...
  public async generate() {
    ...
    if (fileMetadata === null) {
      for (const [path, fileMetadata] of this.#fileMetadataList) {
        if (this.#fileGenerators.has(path)) {
          throw new Exception('Generator already exists');
        }
        this.#fileGenerators.set(
          path,
          new FileGeneratorTypeScript(
            Array.from(this.#fileMetadataList.values()),
            {
      ...
      for (const generator of this.#fileGenerators.values()) {
        generator.generate();
  ...
  async #iterateFileGenerators(
    fn: (f: FileGeneratorTypeScript) => Promise<void> | void
  ) {
      ...
      case NodeType.ImportStatement: {
        ...
        if (externalModule) {
          ...
          if (!mainFileGenerator) {
            mainFileGenerator = new FileGeneratorTypeScript(
              {
                path: mainFile
              },
              {
  ...
  //         case NodeType.TypeDefinition:
  //         case NodeType.CallDefinition:
  //           node.params = Array.from(node.params).sort((p1, p2) =>
    ...
    switch (node.type) {
      ...
      case NodeType.ImportStatement: {
        ...
        if (!fileGenerator) {
          ...
          fileGenerator = new FileGeneratorTypeScript(
            {
              path: inputFile
            },
            {
  ...
  //     /**
  //      * if we're trying to import something from the current @type {FileGeneratorTypeScript}
  ...
  #sourceImportToOutDirImport(
    originalFileGenerator: FileGeneratorTypeScript,
    absolutePath: string
  ) {
  ...
  #generateFinalCode() {
    ...
    if (this.#current === null) {
      // TODO: Create child file generator. This is the root file generator
      return;
    }
    for (const node of this.#current.metadata) {
      this.#generateNodeCode(node);
    }
  }
  ...
  #generateDefinitionCompareFunction(node: IMetadataTypeDefinition) {
    ...
    this.write(
      ...
      () => {
        ...
        this.write(
          ...
          () => {
            for (const p of node.params) {
              this.#writeMultilineComment(`compare parameter ${p.name}`);
              this.write('');
              ...
              if (p !== node.params[node.params.length - 1]) {
                this.append(' &&');
              }
              this.append('\n');
              depth++;
    ...
    switch (resolved.type) {
      ...
      case 'template': {
        switch (resolved.template) {
          ...
          case 'tuple': {
            const exps = resolved.args.map((exp, index) => ({
              exp,
            ...
            for (const { exp, index, varName1, varName2 } of exps) {
              ...
              if (exp !== resolved.args[resolved.args.length - 1]) {
                this.append(' && ');
              }
            }
            break;
      ...
      case 'internalType': {
        ...
        const metadata = this.#current.metadata.find(
          (m) => m.name === resolved.interfaceName
        );
        if (!metadata) {
          throw new Exception('Failed to get metadata');
        }
        this.append(`${getCompareFunctionName(metadata)}(${v1},${v2})`);
        break;
      }
      case 'externalType': {
  ...
  #generateDefinitionDefaultObjectCreator(node: IMetadataTypeDefinition) {
    this.write(
  ...
  #generateDefinitionUpdateFunction(node: IMetadataTypeDefinition) {
    const changesVarName = node.params.length ? 'changes' : '_';
    this.write(
      ...
      () => {
        ...
        for (const p of node.params) {
          this.write(
            `if(typeof changes['${p.name}'] !== 'undefined') {\n`,
            () => {
              this.write('if(!(');
              depth = this.#generateComparisonExpression(
                p.typeExpression,
              ...
              this.indentBlock(() => {
                this.write(
                  `value = ${getTypeDefinitionOrCallDefinitionObjectCreator(
                    node
                  )}({\n`,
                  () => {
                    this.write('...value,\n');
                    this.write(`${p.name}: changes['${p.name}'],\n`);
                  },
                  '});\n'
                );
              });
  ...
  #generateNodeCode(metadata: Metadata) {
    switch (metadata.kind) {
      case 'call':
      case 'type':
        this.#generateDefinitionInterface(metadata);
        this.#generateDefinitionAssertionFunction(metadata);
        this.#generateDefinitionObjectCreatorFunction(metadata);
        this.#generateDefinitionEncodeFunction(metadata);
        this.#generateDefinitionDecodeFunction(metadata);
        this.#generateDefinitionDefaultObjectCreator(metadata);
        this.#generateDefinitionCompareFunction(metadata);
        this.#generateDefinitionUpdateFunction(metadata);
        break;
      case 'trait': {
        this.write(
          `export type ${metadata.name} = ${metadata.nodes
            .map((n) =>
        ...
        this.#generateTraitDefinitionAssertionFunction(
          metadata,
          metadata.nodes
        );
        this.#generateEncodeTraitFunction(metadata, metadata.nodes);
        this.#generateDecodeTraitFunction(metadata, metadata.nodes);
        this.#generateTraitDefaultFunction(metadata, metadata.nodes);
        this.#generateTraitCompareFunction(metadata, metadata.nodes);
        break;
      }
      default:
        throw new Exception(
          `Failed to generate code for metadata: ${
            // @ts-expect-error
            metadata.kind
          }`
        );
  ...
  #generateDefinitionAssertionFunction(node: Metadata) {
  ...
  #resolveTypeExpression(typeExpression: NodeTypeExpression): ResolvedType {
    ...
    if (id instanceof FileGeneratorTypeScript) {
      return {
  ...
  #getTypeDefinitionOrCallDefinitionNamePropertyValue(node: Metadata) {
  ...
  #generateDefinitionEncodeFunction(node: IMetadataTypeDefinition) {
    ...
    const args = [
      '__s: ISerializer',
      `${node.params.length ? 'value' : '_'}: ${interfaceName}`
    ];
    this.write(
  ...
  #generateDefinitionObjectCreatorFunction(
    ...
    const defaultAssignment = node.params.length ? '' : ' = {}';
    const paramsVarName = node.params.length ? 'params' : '_';
    this.write(
  ...
  #generateTypeDefinitionOrCallParameters(node: IMetadataTypeDefinition) {
    for (const p of node.params) {
      const resolvedType = this.#resolveTypeExpression(p.typeExpression);
      this.write(
