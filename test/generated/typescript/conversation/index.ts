...
export function isConversation(value: unknown): value is Conversation {
  if(!(typeof value === 'object' && value !== null && '_name' in value && typeof value['_name'] === 'string' && value['_name'] === "conversation.index.Conversation")) return false;
  if(!(
  ...
  if(!(
    "user" in value && ((__v1) => (isUser(__v1)))(value['user'])
  )) return false;
  return true;
}
...
export function isConversations(value: unknown): value is Conversations {
  if(!(typeof value === 'object' && value !== null && '_name' in value && typeof value['_name'] === 'string' && value['_name'] === "conversation.index.Conversations")) return false;
  if(!(
    "conversations" in value && ((__v0) => ((Array.isArray(__v0) || __v0 instanceof Set) && Array.from(__v0).every(p => (isConversation(p)))))(value['conversations'])
  )) return false;
  return true;
}
...
export function compareConversations(__a: Conversations, __b: Conversations): boolean {
  return (
     ...
     */
    __a['conversations'].length === __b['conversations'].length && Array.from(__a['conversations']).every((__originalItem0,__index0) => (typeof __originalItem0 === 'undefined' ? false : (__item0 => typeof __item0 === 'undefined' ? false : (compareConversation(__originalItem0,__item0)))(Array.from(__b['conversations'])[__index0])))
  );
}
export function updateConversations(value: Conversations, changes: Partial<ConversationsInputParams>) {
  if(typeof changes['conversations'] !== 'undefined') {
    if(!(changes['conversations'].length === value['conversations'].length && Array.from(changes['conversations']).every((__originalItem1,__index1) => (typeof __originalItem1 === 'undefined' ? false : (__item1 => typeof __item1 === 'undefined' ? false : (compareConversation(__originalItem1,__item1)))(Array.from(value['conversations'])[__index1]))))) {
      value = Conversations({
        ...value,
