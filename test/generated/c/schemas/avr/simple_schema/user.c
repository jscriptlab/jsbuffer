...
enum jsb_result_t
simple_schema_user_decode(struct jsb_deserializer_t* d,
                          struct simple_schema_user_t* result) {
  {
    ...
    if (header != -1196640358) {
      JSB_TRACE("simple_schema_user_decode",
                "Invalid CRC header for simple_schema.User. Expected "
                "-1196640358, but got %d instead.",
                header);
  ...
  {
    JSB_CHECK_ERROR(
        jsb_deserializer_read_uint8(d, &result->last_post.has_value));
    if (result->last_post.has_value != 1 && result->last_post.has_value != 0) {
      JSB_TRACE("decode",
                "Failed to decode result->last_post. The returned value for "
    ...
    if (result->last_post.has_value) {
      JSB_CHECK_ERROR(
          simple_schema_post_trait_decode(d, &result->last_post.value));
    }
  }
  return JSB_OK;
}
...
enum jsb_result_t
simple_schema_user_encode(const struct simple_schema_user_t* input,
  ...
  if (input->last_post.has_value) {
    ...
    JSB_CHECK_ERROR(
        simple_schema_post_trait_encode(&input->last_post.value, s));
  } else {
