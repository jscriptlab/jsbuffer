...
protocol_main_get_user_decode(struct jsb_deserializer_t* d,
  ...
  {
    JSB_TRACE("protocol_main_get_user_decode",
              "Decoding protocol.main.GetUser...");
    jsb_int32_t header;
    JSB_CHECK_ERROR(jsb_deserializer_read_int32(d, &header));
    if (header != -1150313593) {
      JSB_TRACE("protocol_main_get_user_decode",
                "Invalid CRC header for protocol.main.GetUser. Expected "
                "-1150313593, but got %d instead.",
                header);
...
protocol_main_get_user_encode(const struct protocol_main_get_user_t* input,
  ...
  JSB_TRACE("protocol_main_get_user_encode",
