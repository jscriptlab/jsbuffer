...
enum jsb_result_t app_command_move_forward_decode(struct jsb_deserializer_t* d, struct app_command_move_forward_t* result) {
  {
    JSB_TRACE("app_command_move_forward_decode", "Decoding app.CommandMoveForward...");
    jsb_int32_t header;
    JSB_CHECK_ERROR(jsb_deserializer_read_int32(d, &header));
    if(header != 1407274108) {
      JSB_TRACE("app_command_move_forward_decode", "Invalid CRC header for app.CommandMoveForward. Expected 1407274108, but got %d instead.", header);
...
enum jsb_result_t app_command_move_forward_encode(const struct app_command_move_forward_t* input, struct jsb_serializer_t* s) {
  JSB_TRACE("app_command_move_forward_encode", "Encoding app.CommandMoveForward...");
...
enum jsb_result_t app_command_move_forward_init(struct app_command_move_forward_t* value) {
  if(value == NULL) {
    JSB_TRACE("app_command_move_forward_init", "Failed to initialize app.CommandMoveForward, received value = NULL.");
    return JSB_BAD_ARGUMENT;
  }
  JSB_TRACE("app_command_move_forward_init", "Initializing param of type \"bool\": stop.");
  /**
   * bool
   */
  value->stop = false;
  ...
  JSB_TRACE("app_command_move_forward_init", "Initializing param of type \"jsb_double_t\": value2.");
  /**
   * jsb_double_t
   */
  value->value2 = 0.0;
