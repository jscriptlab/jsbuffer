...
enum jsb_result_t app_deep_optional_decode(struct jsb_deserializer_t* d, struct app_deep_optional_t* result) {
    {
        JSB_TRACE("app_deep_optional_decode", "Decoding app.DeepOptional...");
        jsb_int32_t header;
        JSB_CHECK_ERROR(jsb_deserializer_read_int32(d, &header));
        if(header != -576678332) {
            JSB_TRACE("app_deep_optional_decode", "Invalid CRC header for app.DeepOptional. Expected -576678332, but got %d instead.", header);
    ...
    JSB_TRACE("app_deep_optional_decode", "Decoding value...");
    {
        ...
        if(result->value.has_value != 1 && result->value.has_value != 0) {
            JSB_TRACE("decode", "Failed to decode result->value. The returned value for optional was not zero or one: %d\n", result->value.has_value);
...
#endif
                JSB_CHECK_ERROR(jsb_deserializer_read_buffer(d, len, result->value.value));
        ...
        if(result->value2.has_value) {
            {
                ...
                if(result->value2.value.has_value) {
                    JSB_CHECK_ERROR(jsb_deserializer_read_int32(d, &result->value2.value.value));
                }
            }
        }
    }
...
enum jsb_result_t app_deep_optional_encode(const struct app_deep_optional_t* input, struct jsb_serializer_t* s) {
...
enum jsb_result_t app_deep_optional_init(struct app_deep_optional_t* value) {
    if(value == NULL) {
        JSB_TRACE("app_deep_optional_init", "Failed to initialize app.DeepOptional, received value = NULL.");
        return JSB_BAD_ARGUMENT;
    }
    JSB_TRACE("app_deep_optional_init", "Initializing param of type \"struct app_deep_optional_string_optional_t\": value.");
    /**
     * struct app_deep_optional_string_optional_t
     */
    value->value.has_value = false;
    /**
     * jsb_string_t
     */
     ...
     *
     * 1. Have additional value->value.value_len and value->value.value_capacity members
...
#else
    jsb_memset(&value->value.value, 0, sizeof(jsb_string_t));
#endif // JSB_SCHEMA_MALLOC
    JSB_TRACE("app_deep_optional_init", "Initialized param: value.");

    JSB_TRACE("app_deep_optional_init", "Initializing param of type \"struct app_deep_optional_int_optional_optional_t\": value2.");
    /**
