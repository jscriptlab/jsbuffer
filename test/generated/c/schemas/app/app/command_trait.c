...
app_command_trait_encode(const struct app_command_trait_t* input,
  ...
  case APP_COMMAND_MOVE_FORWARD_TYPE:
    JSB_TRACE("app_command_trait_encode/app.Command",
              "Encoding app.CommandMoveForward...");
    JSB_CHECK_ERROR(app_command_move_forward_encode(
        &input->value.app_command_move_forward, s));
    break;
  case APP_COMMAND_MOVE_BACKWARDS_TYPE:
    JSB_TRACE("app_command_trait_encode/app.Command",
              "Encoding app.CommandMoveBackwards...");
    JSB_CHECK_ERROR(app_command_move_backwards_encode(
        &input->value.app_command_move_backwards, s));
    break;
  default:
    JSB_TRACE("app_command_trait_encode/app.Command",
...
enum jsb_result_t app_command_trait_decode(struct jsb_deserializer_t* d,
                                           struct app_command_trait_t* output) {
  if (d == NULL) {
    JSB_TRACE("app_command_trait_decode",
              "Failed to decode app.Command, received NULL pointer for the "
  ...
  switch (header) {
  case 1407274108:
    JSB_TRACE("app_command_trait_decode/app.Command",
              "Decoding app.CommandMoveForward...");
    JSB_CHECK_ERROR(app_command_move_forward_decode(
        d, &output->value.app_command_move_forward));
    break;
  case 985001043:
    JSB_TRACE("app_command_trait_decode/app.Command",
              "Decoding app.CommandMoveBackwards...");
    JSB_CHECK_ERROR(app_command_move_backwards_decode(
        d, &output->value.app_command_move_backwards));
    break;
  default:
    JSB_TRACE("app.Command/decode",
...
enum jsb_result_t app_command_trait_init(struct app_command_trait_t* input,
  ...
  if (input == NULL) {
    JSB_TRACE("app_command_trait_init",
              "Failed to initialize app.Command, received NULL pointer for the "
              "input parameter.");
    return JSB_BAD_ARGUMENT;
  }
  ...
  case APP_COMMAND_MOVE_FORWARD_TYPE:
    JSB_TRACE(
        "app_command_trait_init",
        "Initializing with the given header (%d): app.CommandMoveForward...",
        type);
    input->type = APP_COMMAND_MOVE_FORWARD_TYPE;
    return app_command_move_forward_init(
        &input->value.app_command_move_forward);
  case APP_COMMAND_MOVE_BACKWARDS_TYPE:
    JSB_TRACE(
        "app_command_trait_init",
        "Initializing with the given header (%d): app.CommandMoveBackwards...",
        type);
    input->type = APP_COMMAND_MOVE_BACKWARDS_TYPE;
    return app_command_move_backwards_init(
        &input->value.app_command_move_backwards);
  default:
    JSB_TRACE(
...
void app_command_trait_free(struct app_command_trait_t* trait) {
  if (trait == NULL) {
    JSB_TRACE("app_command_trait_free",
              "Failed to free 'app.Command', received NULL pointer.");
    return;
  }
